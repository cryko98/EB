...
const PostActionButton: React.FC<{ icon: React.ReactElement; label: string }> = ({ icon, label }) => {
    return (
        <div className="flex-1 flex items-center justify-center p-2 rounded-lg hover:bg-fb-hover cursor-pointer space-x-2">
            {icon}
            <span className="font-semibold text-fb-secondary-text">{label}</span>
        </div>
    );
};
...
const Post: React.FC<PostProps> = ({ post, user }) => {
  return (
    <div className="bg-fb-card rounded-lg shadow-md">
      ...
      {post.imageUrl && (
        <div className="bg-black -mx-px">
          <img src={post.imageUrl} alt="Post content" className="w-full object-cover" />
        </div>
      )}

      {/* Post Stats */}
      <div className="px-4 py-2 flex justify-between items-center text-fb-secondary-text text-sm">
