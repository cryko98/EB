...
const NavIcon: React.FC<{ path: string; isActive?: boolean }> = ({ path, isActive }) => (
    <div className={`flex-grow flex justify-center p-2 rounded-lg cursor-pointer ${isActive ? 'text-fb-blue border-b-4 border-fb-blue' : 'text-fb-secondary-text hover:bg-fb-input'}`}>
...
const Header: React.FC = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 bg-fb-header shadow z-50 h-14 flex items-center px-4">
      <div className="flex items-center w-full">
        ...
        <div className="flex items-center space-x-2">
            ...
            <img src={currentUser.avatarUrl} alt={currentUser.name} className="w-10 h-10 rounded-full cursor-pointer"/>
        </div>
      </div>
    </header>
  );
